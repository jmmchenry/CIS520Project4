...
void *longest_common_substring_setup(void *rank){
	int startPos = ((intptr_t) rank) * ceil((double)file_line_count / NUM_THREADS);
...
int main(int argc, char *argv[]) {
	...
	for (i = 0; i < NUM_THREADS; i++ ) {
	      rc = pthread_create(&threads[i], &attr, longest_common_substring_setup, (void *)(intptr_t) i);
	      if (rc) {
